...
STATEMENTS_OUT = "statements_start-1900_sec.jsonl"
PROOF_SECTIONS_OUT = "proof_sections_start-1900_sec.jsonl"
...
def extract_statements(latex_content, paper_id, env_map):
    ...
    for env, display in env_map.items():
        ...
        for start, end, _opt_arg, content in spans:
            ...
            content = strip_proofs(content).strip()
...
def extract_proofs(latex_content, paper_id):
    """Find all proofs. Returns list of dicts (unsorted)."""
    proofs = []
...
def find_body_end(latex_content):
    """Find the position where the body ends and bibliography start or \\end{document}"""
    # TODO

    return

def extract_proof_section(latex_content, paper_id):
...
def assign_ids_and_attach(statements, proof_sections, paper_id):
    """Sort by document position, assign stable ids, attach each proof sections to the statement it refers to."""
    ...
    return statements, proof_sections
################################################################################
...
def process_tex_directories():
    ...
    papers_processed = papers_failed = n_statements = n_proofs = 0

    with open(STATEMENTS_OUT, "w", encoding="utf-8") as sf, \
         open(PROOF_SECTIONS_OUT, "w", encoding="utf-8") as pf:
        ...
        for dirpath in pbar:
            ...
            try:
                ...
                statements = extract_statements(cleaned, paper_id, env_map)
                proof_sections = extract_proof_section(cleaned, paper_id)
                statements, proof_sections = assign_ids_and_attach(statements, proof_sections, paper_id)
    ...
    print(f"Proof Sections found:     {n_proofs}")
    print(f"Proof Sections ignored:     {ignored_counter}")
    print(f"Statements -> {STATEMENTS_OUT}")
    print(f"Proof Sections     -> {PROOF_SECTIONS_OUT}")


if __name__ == "__main__":
    process_tex_directories()

#     text = r"""\section{Proof of Theorem \ref{thm:minimax}}\label{app:proofminimax}
# Here we focus on the case of $\alpha =2$ and use the standard technique of hypercube construction to establish the lower bound \cite{CaDo02Illposed}. Let $\phi:[0,1] \rightarrow \mathds{R}^+ \cup \{0\}$ be a two-times differentiable function with $\|\frac{d^2 \phi}{dt^2} \|_{\infty}=1$, $\phi(0) = \phi(1) =0$, $\left. \frac{d\phi}{dt} \right|_0 = \left. \frac{d\phi}{dt} \right|_{1}=0$, and $\left. \frac{d^2\phi}{dt^2} \right|_0 = \left. \frac{d^2\phi}{dt^2} \right|_{1}=0$. For $ i = 0, 1, \ldots, m-1$ define


# \section{Proof of Theorem \ref{main}}\label{proof}

# First we show that it is sufficient to prove Theorem \ref{main} with
# $\W_{N,k}=\x_{N,k}$.

# \section{end}
# """

    # print(extract_proof_section(text, 1))
    # print(list(find_sections(text1)))
